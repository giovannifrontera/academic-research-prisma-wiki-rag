...
import pytest
from wiki_qdrant import (get_db, ensure_table, upsert, query_similar, promote_staging,
                          rollback_staging, detect_renames, list_tables)
...
def test_ensure_table_creates_table(tmp_workspace):
    db = get_db(str(tmp_workspace / "memory" / "qdrant"))
    ensure_table(db)
    assert "wiki_pages" in list_tables(db).tables
...
def test_find_semantic_duplicates_empty(tmp_path):
    from wiki_qdrant import get_db, find_semantic_duplicates
    db = get_db(str(tmp_path / "qdrant"))
...
def test_find_semantic_duplicates_detects_near_identical(tmp_path):
    import numpy as np
    from wiki_qdrant import get_db, upsert, find_semantic_duplicates
